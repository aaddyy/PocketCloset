...
void main() {
    ...
    for (int i=0; i<3; i++)
        foregroundColor[i] = clamp(foregroundColor[i], 0.0, 1.0);
    

    //vec3 foreground = queryColor.xyz;
...
/*
    for (int i=0; i<3; i++)
        foreground[i] = clamp(foreground[i], 0.0, 1.0);
    alpha = clamp(alpha, 0.0, 0.95);
    
    vec4 backgroundColor = gl_LastFragData[0];
*/
...
//	alpha = compress(alpha, 0.7);

	vec4 src = vec4(foreground * alpha, alpha);
    vec4 dst = gl_LastFragData[0];
    vec4 maxBlend = max(src, dst);
//    vec4 addBlend = src + dst * (1.0 - src.a);

    float weightSum = src.w + dst.w;
    weightSum = weightSum > 0.01 ? weightSum : 1.0;
	vec4 avgBlend;
/*    avgBlend.w = mix(
    	max(src.w, dst.w),
    	(src.w + dst.w) / 2.0,
    	smoothstep(0.1, 1.0, min(src.w, dst.w)));*/
    
    avgBlend.w = max(src.w, dst.w);
    avgBlend.xyz = (src.xyz + dst.xyz) / weightSum * avgBlend.w;

	gl_FragData[0] = maxBlend;
//	gl_FragData[0] = avgBlend;
//    gl_FragData[0] = vec4(strokeCoords, 0.0, 1.0);

//    gl_FragColor = vec4(_strokeCoords.x * 0.5 + 0.5, _strokeCoords.y / 255.0, 0.0, 1.0);
}
